...
type Route = { name: "home" } | { name: "patient"; id: string } | { name: "members" } | { name: "plans" } | { name: "today" };
...
export class AppController {
  ...
  private activeView(): HTMLElement {
    switch (this.route.name) {
      ...
      case "today": return renderToday(this);
    }
  }
  ...
  private topbar(): HTMLElement {
    ...
    if (atHome) {
      right.push(el("button", { class: "iconbtn", "aria-label": "Today's tasks", onclick: () => this.navigate({ name: "today" }) }, [icon("calendar", 22)]));
    }
  ...
  private routeTitle(): string {
    ...
    if (this.route.name === "today") return "Today's tasks";
    return "Physio";
  }
